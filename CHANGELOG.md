...
- **NEW**: Introducing CachedWatcher.
    - CachedWatcher is a wrapper around the standard Watcher, with local caching capabilities, Ideal for persisting
      state between app sessions.
    - It efficiently stores and retrieves values with a unique key, utilizing `write` and `onRead` functions for
      enhanced data handling.

- **BREAKING CHANGES**:
  In an effort to enhance clarity and consistency of the `Watcher`, we've made the following changes:
    - **Watch Initialization:**
        - `Watch<T>` is now renamed to `Watcher<T>`.
        - The `.watch` getter for creating `Watcher<T>` instances has been updated to `.watcher`.
        - Similarly, `BoolWatch`, `StringWatch`, `IntWatch`, etc., are now `BoolWatcher`, `StringWatcher`, `IntWatcher`,
          and so on.
    - **Widget Renaming and Functional Update:**
        - The `ValueWatcher` widget is now `ValueWatch`.
        - The `.watcher` widget getter is updated to `.watch`.
          This change simplifies the syntax and improves code readability.
        - We've introduced `watchWhen` as an optional parameter in the ValueWatch widget,
          enhancing its flexibility and control over UI updates.

For a detailed showcase of these breaking changes, please refer to our [Migration Guide]()

- **ENHANCES**: `indexOfOrNull` now support taking null values.

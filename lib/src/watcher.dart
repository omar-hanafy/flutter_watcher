...
extension PrivateHelper<T> on Watcher<T> {
  bool _isEqual(T value1, T value2) {
    if (value1 is List && value2 is List) return listEquals(value1, value2);
    if (value1 is Set && value2 is Set) return setEquals(value1, value2);
    if (value1 is Map && value2 is Map) return mapEquals(value1, value2);
    return value1 == value2;
  }

  R updateOnAction<R>(R Function() action, {bool refresh = true}) {
    final current = v;
    final a = action();
    if (_isEqual(current, v)) {
      prevValue = current;
      notifyListeners();
    } else if (refresh) {
      notifyListeners();

...
extension SetWatcherExtension<E> on Watcher<Set<E>> {
  ...
  ///
  /// Returns `true` if [value] (or an equal value) was not yet in this [Watcher]'s value.
  /// Otherwise returns `false` and this [Watcher]'s value is not changed.
  ///
  /// Example:
  /// ```dart
  ...
  /// ```dart
  /// final charactersWatcher = <String>{'A', 'B'}.watcher;
  /// characters.addAll({'A', 'B', 'C'});
  /// print(characters); // {A, B, C}
  /// ```
  void addAll(Set<E> elements) => updateOnAction(() => value.addAll(elements));

  /// Removes [value] from this [Watcher]'s value.
  ///
  /// Returns `true` if [value] was in this [Watcher]'s value, and `false` if not.
  /// The method has no effect if [value] was not in this [Watcher]'s value.
  /// ```dart
  /// final charactersWatcher = <String>{'A', 'B', 'C'}.watcher.watcher;
  ...
  ///
  /// Checks whether [object] is in this [Watcher]'s value, like [contains], and if so,
  /// returns the object in this [Watcher]'s value, otherwise returns `null`.
  ///
  /// If the equality relation used by this [Watcher]'s value is not identity,
  ...
  /// equal element in this [Watcher]'s value is retained, and elements that are not equal
  ...
  /// ```dart
  /// final characters1Watcher = <String>{'A', 'B', 'C'}.watcher;
  ...
  /// Example:
  /// ```dart
  /// var planetsWatcher = <String>{'Earth', 'Jupiter'}.watcher;
  /// var updated = planets.followedBy({'Mars', 'Venus'});
  /// print(updated); // (Earth, Jupiter, Mars, Venus)
  /// ```
  Iterable<E> followedBy(Set<E> other) =>
  ...
  ///
  /// Returns true if this [Watcher]'s values are both null, or if they are both non-null, have
